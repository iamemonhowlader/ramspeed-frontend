...
import ImageUpload from "../../change-logo/components/ImageUpload";
import RMAUserLoginPopUp from "./RMAUserLoginPopUp";
import useAuthStore from "@/store/authStore";
import useRmaStore from "@/store/rmaStore";
...
export default function RMAForm() {
  ...
  const { rmaFormData, setRmaFormData } = useRmaStore();
  ...
  const handleCustomerSignIn = () => {
    ...
    setRmaFormData(formData);
    setRedirectPath("/administrator/dashboard/shop-management/rma-form");
  ...
  useEffect(() => {
    if (
      rmaFormData &&
      rmaFormData?.beforeImage?.name &&
      rmaFormData?.afterImage?.name
    ) {
      setFormData(rmaFormData);
    }
    setRedirectPath("/");
    setRmaFormData(null);
  }, []);
      ...
      <div className="mx-auto bg-white rounded-2xl border-[#dedede] shadow-xl p-6 md:p-8 gap-12 flex flex-col">
        ...
        {(formData.passwordType === "pin" ||
          ...
          <div>
            ...
            <input
              type={formData.passwordType === "pin" ? "number" : "password"}
              placeholder={
                formData.passwordType === "pin"
                  ? "Enter PIN code"
                  : "Enter password"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows="4"
              className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition resize-none ${
                formErrors.repairStatus ? "border-red-500" : "border-gray-300"
              }`}
            />
        ...
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition ${
              formErrors.repairCost ? "border-red-500" : "border-gray-300"
            }`}
          />
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-end">
          ...
          {customer ? (
            ...
            <Button onClick={() => handleCustomerSignIn()}>
              Sign in to proceed
            </Button>
          )}
